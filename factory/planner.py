"""Idea discovery and duplicate-aware selection."""
from __future__ import annotations

from .history import is_duplicate
from .models import Idea
from .provider import available, discover_ideas


def reviewed_fallback() -> list[Idea]:
    return [
...
def discover(history: list[dict] | None = None, use_provider: bool = False) -> list[Idea]:
    """Return AI candidates when explicitly enabled and configured, else reviewed fallback ideas.

    Tests call this with defaults, so repository validation never depends on network access.
    """
    if use_provider and available():
        return discover_ideas(history or [])
...
def select(candidates: list[Idea], history: list[dict], existing_slugs: set[str]) -> tuple[Idea | None, list[dict]]:
    ...
    for idea in candidates:
        ...
        if idea.slug in existing_slugs:
            duplicate, score, matched = True, 1.0, idea.slug
        if duplicate:
